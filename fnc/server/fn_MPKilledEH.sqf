					...
					else
					{
						...
						else
						{
							if (_killed isKindOf "Air") then
								...
								{ if (_killed isKindOf "Ship") then {_teampoints = 2;}; };
						};
					};
				};
			};
	...
	if (vehicle _killer != _killer) then
	{
		...
		if (isPlayer _driverveh) then
		{
			if (_killer != _driverveh) then
				{ if (alive _driverveh) then {_driverveh addScore _teampoints}; };
		};
		if (isPlayer _gunnerveh) then
		{
			if (_killer != _gunnerveh) then
				{ if (alive _gunnerveh) then {_gunnerveh addScore _teampoints}; };
		};
		if (isPlayer _commanderveh) then
		{
			if (_killer != _commanderveh) then
				{ if (alive _commanderveh) then {_commanderveh addScore _teampoints}; };
		};
	};
