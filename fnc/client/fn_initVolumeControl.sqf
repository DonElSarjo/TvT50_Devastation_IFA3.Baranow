...
kbn_global_soundLast = 0; //для приглушения 50% громкости
...
//фцнкция приглушения 50% громкости
...
//подменим функцию ace_hearing_fnc_updateVolume
_fnc_findHandler =
{
...
private["_handle"];
waitUntil{_handle = call _fnc_findHandler; _handle >= 0}; //найдем обработчик

[_handle] call CBA_fnc_removePerFrameHandler; //удалим обработчик
...
ace_hearing_fnc_updateVolume_New =
{
	...
	ace_hearing_volume = ace_hearing_volume * kbn_global_soundVolume; //kaban
	...
	_volume = _volume * ace_hearing_volumeAttenuation; //kaban

	_volumeForRadio = _volume / kbn_global_soundVolume; //kaban

	if (ACE_player getVariable ["ACE_isUnconscious", false]) then {
		_volume = _volume * ace_hearing_UnconsciousnessVolume; //kaban
		_volumeForRadio = _volumeForRadio 	* ace_hearing_UnconsciousnessVolume; //kaban
	};
	...
	ACE_player setVariable ["tf_globalVolume", _volumeForRadio]; //kaban
	if (!isNil "acre_api_fnc_setGlobalVolume") then {[_volumeForRadio^(0.33)] call acre_api_fnc_setGlobalVolume;}; //kaban
};

[ace_hearing_fnc_updateVolume_New, 1, [false]] call CBA_fnc_addPerFrameHandler; //добавим обработчик с измененной функцией
