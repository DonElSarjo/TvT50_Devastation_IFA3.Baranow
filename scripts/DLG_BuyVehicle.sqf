...
_canbuy = true;
{
	...
	if !(isNil "_vehname") then
		{ if (_vehname == (getPlayerUID player)) then { if (alive _x) then {_canbuy = false;_vehicle = _x;}; }; };
} count vehicles;
...
if (RIPPLAYERSIDE == 0) then {_vehlist = AVAILABLEVEHICLEE} else {_vehlist = AVAILABLEVEHICLEW};
...
ctrlSetText [5923, format[localize "STR_RIP_DLGMONEY",_money]];
{
	_cfg = switch (true) do
	{
		case (isClass(configFile >> "CfgWeapons" >> (_x select 0))): {configFile >> "CfgWeapons" >> (_x select 0)};
		case (isClass(configFile >> "CfgMagazines" >> (_x select 0))): {configFile >> "CfgMagazines" >> (_x select 0)};
		case (isClass(configFile >> "CfgVehicles" >> (_x select 0))): {configFile >> "CfgVehicles" >> (_x select 0)};
	};
